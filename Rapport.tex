...
\chapter{Présentation du projet}
  \section{Sujet}
  L'objectif de ce projet est de réaliser un outil permettant d'exécuter des algorithmes distribués sur des tablettes et des téléphones portables. Pour ce faire, les appareils doivent pouvoir communiquer de manière synchronisée. Il doit être possible de rédiger simplement des algorithmes afin de les dérouler et d'observer leur comportement (visualiser les messages et leur contenu). \todo{WTF ->} Ces algorithmes ont vocation à être utilisés pour créer des applications distribuées.
  ...
  et \todo{WTF ->} la réalisation de démonstrations pour simplifier leur compréhension. Le travail
...
\chapter{Recherche des technologies de communication}
  ...
  \section{Wifi Direct}
    ...
    \subsection{Fonctionnement}
     \paragraph{}
    L'activation du Wifi Direct sur les appareils ne nécessite d'activer qu'une option dans les paramètres du Wifi, grâce aux options avancé. Une fois ceci fait, l'envoi des fichiers est possible.
    \paragraph{}
    Si la technologie n'est pas activée au lancement d'une application, une autorisation d'activation est possible à mettre en place dans le code afin de mettre en marche le Wifi Direct. En revanche ce n'est utilisable que sur des dispositifs sous Android 2.3 minimum.
    \subsection{Avantages}
     \paragraph{}
    ...
    \subsection{Inconvénients}
    \paragraph{}
    D'après certains retours, l'activation du Wifi Direct peut entraîner des soucis sur certains modèles. De plus certains dispositifs ne fonctionnent qu'avec des appareils de la même marque, ce qui complique la mise en place d'un algorithme distribué.
    L'écriture du code du Wifi Direct n'est pas aisée à cause du manque de documentation et du fait que cette technologie n'est pas encore très répendue. \todo{plutôt à mettre dans la partie du choix, ici on se contente de résumer ce qu'on a appris juste avec la recherche initiale sans commencer à coder} D'autre part, lorsque des erreurs étaient rencontrées lors de l'écriture de l'algorithme \todo{pas algo, application ?}, il était difficile de savoir d'où provenait le problème. Est-ce qu'il était dû à la marque du téléphone ? Ou à une erreur de code ? 
  \section{NFC}
    \subsection{Historique}
     \paragraph{}
    ...
    \subsection{Avantages}
    \paragraph{}
    Le NFC dispose de nombreux avantages comme des fréquences très basse (13.56Mhz contre 2.4Ghz pour le Wifi b/g/n/ac) permettant de \todo{demander a celia ce que ca fait, TAIN MAIS VOUS ETES SERIEUX ?}et un mode de communication full duplex permettant des communications dans les deux sens en même temps.
    \subsection{Inconvénients}
    \paragraph{}
    Le principal inconvénient de cette technologie est la distance, en effet elle ne fonctionne qu'avec un espacement de quelques centimètres entre les deux terminaux. \todo{idem, à mettre dans le choix} Cela ne convenait pas à la constructions d'un grand réseau de téléphones portables.
  \section{Choix de la technologie}
  
  Afin de prendre une décision sur la techonolie a utilisé pour l'application, des comparatifs ont été réalisés en mettant en avant les avantages et les inconvénients. 
  \paragraph{}
  ...
  \paragraph{}
  Pour départager le Bluetooth et le Wifi Direct, il a été décidé de coder deux applications utilisant chacune une des technologie et de les comparer directement grâce aux implémentation réaliser.
  \paragraph{}
  Concernant le Bluetooth, l'avancé de l'application était plutôt rapide, grâce à des tutoriels Android mise en place par Google. Cependant pour le Wifi Direct, l'écriture du code a été plus compliqué à réaliser, lorsque des erreurs étaient rencontrées lors de l'écriture de l'application, il était difficile de savoir d'où provenait le problème. Est-ce qu'il était dû à la marque du téléphone ? Ou à une erreur de code ? 
  \paragraph{} 
